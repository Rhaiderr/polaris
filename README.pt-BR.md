...
5. Categorias em `/config/polaris/<email>/categorias.yaml` (criado com um
   exemplo). Serviços: `polaris.run_triage` (triagem; modo `full` p/ backlog),
   `polaris.suggest_categories` / `polaris.accept_categories` (sugestões por IA),
   sensor de última execução e evento `polaris_run_completed` para automações.
