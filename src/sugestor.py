...
Fluxo (pensado para virar tela de onboarding no futuro front/add-on):
 ...
 5. as aceitas entram no config/<conta>/categorias.yaml com defaults seguros
...
def _prompt_checkbox(sugestoes: list[dict]) -> list[dict]:
    ...
    for tok in escolha.split(","):
        ...
        if tok.isdigit() and 1 <= int(tok) <= len(sugestoes):
            aceitas.append(sugestoes[int(tok) - 1])
    return aceitas


def salvar_json(conta: str, logs_dir: str, sugestoes: list[dict]) -> str:
...
def carregar_json(conta: str, logs_dir: str) -> list[dict]:
    path = os.path.join(logs_dir, conta, "sugestoes.json")
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"{path} não existe. Rode antes: --conta {conta} --sugerir-categorias"
        )
